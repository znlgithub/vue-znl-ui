...
<script>
  import ChangeLog from '../../../STANDARD.zh-CN.md';
  ...
  export default {
    ...
    mounted() {
      ...
      let fragments = `<li><h3><a>${release}</a></h3>`;
      ...
      for (let len = changeLogNodes.length, i = 2; i < len; i++) {
        ...
        if (node.tagName !== 'H3') {
          ...
          fragments += `</li><li><h3><a>${release}</a></h3>`;
        }
      }
      fragments = fragments.replace(/#(\d+)/g, '<a>#$1</a>');
      fragments = fragments.replace(/@(\w+)/g, '<a>@$1</a>');
